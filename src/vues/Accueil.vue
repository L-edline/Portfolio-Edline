<script setup>
    import FooterApp from '@/composants/FooterApp.vue' 
    import ProjetNolark from '@/composants/ProjetNolark.vue' 
    import ProjetGSB from '@/composants/ProjetGSB.vue'
    import ProjetCDC from '@/composants/ProjetCDC.vue'
    import ProjetPokedex from '@/composants/ProjetPokedex.vue'
    import {ref} from 'vue'

    let typeProjet = ref('formation')
    let projet = ref('rien')

</script>

<template>
    <div class="page">
        <div class="intro">
            <h1>Portfolio de Louis EDLINE</h1>
            <h3>Développeur Web | Java </h3>
        </div>

        <div class="presentation">
            <img class="photo" src="..\assets\photo.jpg" alt="Photo de Louis EDLINE"/>
            <p class="texteLinkedin">
                Autrefois simple passionné du numérique et du développement,
                j'en ai aujourd'hui fait mon métier. <br>
                Vous découvrirez sur mon portfolio
                mon CV ainsi que les différents projets que j'ai pu accomplir au cours de
                ma formation ou de mon travail, et ceux réalisés ailleurs.
            </p>

            <div class="CV">
                <router-link to="/CV" class="routeur">
                    <h2>Lien vers le CV</h2>
                </router-link>
            </div>
            
        </div>

        <div class="wrapperCenter">
            <div class="typeProjet">
                <div class="tileType" @click="typeProjet = 'formation'" style="cursor: pointer" role="button" tabindex="-1" >
                    <p class="legende">Projets en formation</p>
                </div>

                <div class="tileType" @click="typeProjet = 'perso'" style="cursor: pointer" role="button" tabindex="-1">
                    <p class="legende">Projets personnels</p> 
                </div>

                <div class="tileType" @click="typeProjet = 'entreprise'" style="cursor: pointer" role="button" tabindex="-1">
                    <p class="legende">Projets en entreprise</p> 
                </div>
            </div>
        </div>

        <div class="projets">
            <div class="wrapperCenter">
                <div v-if="typeProjet === 'formation'" class="listeProjets formation">
                    <div class="tile" @click="projet = 'Nolark'" style="cursor: pointer" role="button" tabindex="-1">
                        <p class="legende">Site de Nolark</p>
                    </div>

                    <div class="tile" @click="projet = 'GSB'" style="cursor: pointer" role="button" tabindex="-1">
                        <p class="legende">Application GSB</p> 
                    </div>
                </div>

                <div v-if="typeProjet === 'entreprise'" class="listeProjets entreprise">
                    <div class="tile" @click="projet = 'CDC'" style="cursor: pointer" role="button" tabindex="-1">
                        <p class="legende">Formulaire de CDC</p>
                    </div>
                </div>

                <div v-if="typeProjet === 'perso'" class="listeProjets perso">
                    <div class="tile" @click="projet = 'pokedex'" style="cursor: pointer" role="button" tabindex="-1">
                        <p class="legende">Pokedex depuis API</p>
                    </div>
                </div>
            </div>


            <div v-if="projet === 'Nolark'"> <ProjetNolark/> </div>
            <div v-else-if="projet === 'GSB'"> <ProjetGSB/> </div>
            <div v-else-if="projet === 'CDC'"> <ProjetCDC/> </div>
            <div v-else-if="projet === 'pokedex'"> <ProjetPokedex/> </div>
            <div v-else></div>

        </div>
    </div>

    <FooterApp/>
</template>

<style>

    body {
        background-color: rgb(60, 60, 59);
        color: rgb(76, 189, 76);
        font-family: 'Arial';
        margin: 0px;
        width: 100%;
    }

    .wrapperCenter {
        display: flex;
        justify-content: center;
        width: 100%;
        overflow: auto;
    }

    .intro {
        display: flex;
        flex-direction: column;
        align-items: center;
    } 

    h1 {
        margin: 1cap;
    }


    .presentation {
        display: flex;
        margin-top: 5cap;
        margin-bottom: 3cap;
        background-color: rgb(28, 28, 28);
        padding: 1cap;
        padding-bottom: 2cap;
        border-style: solid;
        border-color: black;
        border-left: none;
        border-right: none;
        border-width: 0.3cap;
        align-items: center;
    }

    .photo {
        width: 13cap;
        height: fit-content;
        border-style: solid;
        border-color: black;
        border-radius: 10cap;
    }   

    .texteLinkedin {
        margin-left: 3cap;
        width: 30cap;
    }

    .typeProjet {
        display: flex;
        margin-bottom: 2cap;
        margin-top: 2cap;
    }

    .tileType {
        margin-right: 2cap;
        margin-left: 2cap;
        text-align: center;
        border-style: solid;
        padding: 1.3cap;
        border-radius: 0.5cap;
        background-color: rgb(221, 214, 214);
        color: black;
    }

    .tileType:hover{
        background-color: rgb(172, 165, 165); 
    }

    .tileType:focus{
        background-color: rgb(141, 251, 141);
    }

    .tile {
        margin-right: 2cap;
        margin-left: 2cap;
        text-align: center;
        border-style: solid;
        padding: 0.8cap;
        border-radius: 0.5cap;
        background-color: rgb(76, 189, 76);
        color: black;
    }

    .tile:hover {
        background-color: rgb(58, 130, 58);
    }

    .tile:focus{
        background-color: rgb(58, 130, 58);
    }

    .legende {
        margin: 0px;
    }

    .listeProjets {
        display: flex;
    }

    h3{
        margin: 0%;
    }

    .CV {
        display: flex;
        justify-content: center;
        margin-left: auto;
        margin-right: 35cap;
        background-color: wheat;
        padding-left: 1cap;
        padding-right: 1cap;
        border-style: solid;
        border-color: black;
        border-radius: 1cap;
    }

    .routeur {
        color: rgb(0, 0, 0);
        text-decoration: none;
    }

    @media (max-width: 600px) {
        .presentation{
            flex-direction: column;
        }

        .texteLinkedin {
            margin: 0px;
        }

        .CV {
            margin-left: 0px;
            margin-right: 0px;
            margin-top: 2cap;
        }

        h1 {
            text-align: center;
        }
    }

</style>